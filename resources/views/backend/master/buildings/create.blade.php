<x-app-layout>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 md:p-8 text-gray-900 dark:text-gray-100" x-data="buildingForm()">
                    <form @submit.prevent="saveBuilding()">
                        <div class="space-y-6">
                            <div>
                                <label for="name_building" class="block text-sm font-medium">Nama Gedung</label>
                                <div class="relative mt-1">
                                    <div class="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                                        <i class="fas fa-building text-gray-400"></i>
                                    </div>
                                    <input type="text" x-model="formData.name_building" id="name_building"
                                        class="block w-full ps-10 border-gray-300 rounded-md"
                                        placeholder="Contoh: Gedung Tower A" required>
                                </div>
                            </div>
                            <div>
                                <label for="address" class="block text-sm font-medium">Alamat (Opsional)</label>
                                <div class="relative mt-1">
                                    <div
                        ...
                        <div class="mt-8 flex justify-end space-x-3">
                            <a href="{{ route('master.buildings.index') }}"
                                class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase">Batal</a>
                            <x-primary-button type="submit" ::disabled="isSubmitting"><span
                                    x-show="!isSubmitting">Simpan</span><span x-show="isSubmitting">Menyimpan...</span>
                            </x-primary-button>
                        </div>
                    </form>
                </div>
    ...
    <script>
        function buildingForm() {
                return {
                    isSubmitting: false,
                    formData: { name_building: '', address: '', status: 'active' },
                    init() { $('#status').select2({ theme: "classic", width: '100%', minimumResultsForSearch: Infinity }); },
