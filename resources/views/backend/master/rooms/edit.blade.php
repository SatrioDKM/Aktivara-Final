<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            ...
            {{ __('Edit Ruangan: ') . $data['room']->name_room }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-lg sm:rounded-lg">
                <div class="p-6 md:p-8" x-data="roomForm(@js($data['floors']))" x-init="initData(@js($data['room']))"
                    x-cloak>
        ...
        function roomForm(allFloors) {
            return {
                ...
                initData(room) {
                    ...
                    this.$nextTick(() => {
                        $('#building_id').select2({ theme: "classic", width: '100%' })
                            .on('change', function() { self.updateFloorOptions($(this).val()); });

                        $('#floor_id').select2({ theme: "classic", width: '100%' })
                            .on('change', function() { self.formData.floor_id = $(this).val(); });

                        $('#status').select2({ theme: "classic", width: '100%', minimumResultsForSearch: Infinity })
                            .on('change', function() { self.formData.status = $(this).val(); });

                        // Set initial values after Select2 is initialized
                        $('#building_id').val(room.floor.building_id).trigger('change');
                        this.updateFloorOptions(room.floor.building_id, room.floor_id); // Update floors and set selected
                        $('#status').val(room.status).trigger('change');
                    });
                },

                updateFloorOptions(buildingId, selectedFloorId = null) {
                ...
                saveRoom() {
                    ...
                    axios.put(`/api/rooms/${this.formData.id}`, this.formData)
                    .then(response => {
                        sessionStorage.setItem('toastMessage', 'Data ruangan berhasil diperbarui!');
